...
@click.option(
    ...
    "-e",
    is_flag=True,
    help="Set this if you have an Enviro (not Enviro+) so don't have gas and particulate sensors",
)
@click.option(
    "--pms",
    "-s",
    is_flag=True,
    help="Enable pms5003 data collection [default: False]",
)
@click.option(
    "--version",
    "-v",
    is_flag=True,
    help="Prints version number and exits [default: False]",
)
def main(bind, port, debug, version, pms, enviro, luftdaten, influxdb, factor):
    if version:
    ...
    if factor:
        logging.info(
            f"""Using compensating algorithm (factor={factor}) to account for
        heat leakage from Raspberry Pi board"""
        )

    if influxdb:
        # Post to InfluxDB in another thread
        logging.info(
            """Sensor data will be posted to InfluxDB every
        {INFLUXDB_TIME_BETWEEN_POSTS} seconds"""
        )
    ...
    if luftdaten:
        ...
        LUFTDATEN_SENSOR_UID = "raspi-" + get_serial_number()
        logging.info(
            f"""Sensor data will be posted to Luftdaten every
        {LUFTDATEN_TIME_BETWEEN_POSTS} seconds for the UID {LUFTDATEN_SENSOR_UID}"""
        )
